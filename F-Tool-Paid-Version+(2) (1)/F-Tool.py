...
# DEVELOPED BY F34RL3SS x FDc0d3 & Aya

from utils.banner import Count, Bot, AttackSentL4, AttackSentVIPL4, AttackSentL7
from utils.banner import HomeMenu, Proxy, WebTools, Spdtest, MenuL4, MenuL7
...
class Tool(Home):
	...
	def spdtest(self):
		...
		try:
			...
			spdt.get_servers(); time.sleep(0.1)

			print(Color.LC+"[*] Choosing Best Server...")
			get = spdt.get_best_server(); time.sleep(0.1)
			...
			print("\n")
		except KeyboardInterrupt:	status.returnH()
		except:	print(Color.LR+"Error: Check your Internet Connection.\n\n")
...
def main():
	...
	status.Style(obj="[+] Checking Dependencies...\n")
	pkgs = ['screen', 'node']
	install = True
	for pkg in pkgs:
		ur_mom = which(pkg)
